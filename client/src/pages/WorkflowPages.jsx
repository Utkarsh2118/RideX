...
function RideHistoryPage() {
  ...
  return <section className="workflow-page"><Link className="back-link" to="/dashboard/passenger"><ArrowLeft size={15} /> Back to booking</Link><p className="eyebrow">PASSENGER HISTORY</p><h1>Your rides.</h1>{notice && <p className="notice">{notice}</p>}{payments.length > 0 && <div className="payment-summary"><span>PAYMENT HISTORY</span><strong>{payments.length} payment{payments.length === 1 ? '' : 's'} · Rs {payments.reduce((total, payment) => total + payment.amount, 0).toFixed(0)}</strong></div>}{rides.length ? <div className="history-list">{rides.map((ride) => <article className="history-row" key={ride.id}><div><strong>{ride.vehicleType} ride · Rs {ride.fare.toFixed(0)}</strong><span>{ride.distanceKm} km · {ride.paymentMethod} · {new Date(ride.createdAt).toLocaleDateString()}</span></div><div className="history-actions"><span className={`status status-${ride.rideStatus.toLowerCase()}`}>{ride.rideStatus.replaceAll('_', ' ')}</span>{['REQUESTED', 'SEARCHING_DRIVER', 'DRIVER_ASSIGNED', 'DRIVER_ARRIVING', 'DRIVER_ARRIVED'].includes(ride.rideStatus) && <button type="button" className="quiet-button" onClick={() => cancel(ride.id)}>Cancel</button>}{ride.rideStatus === 'RIDE_COMPLETED' && ride.paymentMethod === 'online' && ride.paymentStatus !== 'paid' && <button type="button" onClick={() => payOnline(ride.id)}>Pay online</button>}{ride.rideStatus === 'RIDE_COMPLETED' && !ratedRides.includes(ride.id) && <button type="button" className="quiet-button" onClick={() => rateRide(ride.id)}>Rate ride</button>}</div></article>)}</div> : !notice && <p className="empty-state">No rides yet. Your completed trips will appear here.</p>}</section>
}
